...
pair(R, _, _)    --> integer(R), space, "red".
pair(_, _, B)    --> integer(B), space, "blue".
pair(_, G, _)    --> integer(G), space, "green".

record(R, G, B)  --> pair(R, G, B).
record(R, G, B)  --> pair(R, G, B), pair_seperator, record(R, G, B).

turn(R, G, B)    --> record(R, G, B).
turn(R, G, B)    --> record(R, G, B), turn_seperator, turn(R, G, B).

game(R, G, B)    --> "Game ", integer(1000), ": ", turn(R, G, B).

find_failure_red(Game, Limit) :-
    between(0, 100, Red),
    Limit = Red,
    \+ (phrase(game(Red, 100, 100), Game) -> false ; true).

find_failure_blue(Game, Limit) :-
    between(0, 100, Blue),
    Limit = Blue,
    \+ (phrase(game(100, Blue, 100), Game) -> false ; true).

find_failure_green(Game, Limit) :-
    between(0, 100, Green),
    Limit = Green,
    \+ (phrase(game(100, 100, Green), Game) -> false ; true).

find_failure(Game, R) :-
    find_failure_red(Game, RL),
    find_failure_blue(Game, BL),
    find_failure_green(Game, GL),
    R = [RL, BL, GL].
...
main() :-
  ...
  maplist(find_failure, INPUT, Failures),
  maplist(calc_power, Failures, N),
  sum_list(N, Sum),
  writeln(Sum).  