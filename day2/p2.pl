...
number(N) --> digits(Ds), { number_chars(N, Ds), N > 0 }.
digits([D]) --> digit(D).
digits([D|Ds]) --> digit(D), digits(Ds).

color_pair(red, N) --> number(N), space, "red".
color_pair(blue, N) --> number(N), space, "blue".
color_pair(green, N) --> number(N), space, "green".

record([CP]) --> color_pair(_, CP).
record([CP1,CP2|CPRest]) --> color_pair(_, CP1), pair_separator, record([CP2|CPRest]).

turn([R]) --> record(R).
turn([R1|R2Rest]) --> record(R1), turn_separator, turn(R2Rest).

game(RGB) --> "Game ", number(_), ": ", turn(RGB).

t() :- phrase(game(RGB), "Game 1: 6 green, 3 blue; 3 red, 1 green; 4 green, 3 red, 5 blue").
