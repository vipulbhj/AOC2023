...
in_range(X) :- X >= 0, X < 19600.

concatenate_lists([], Result, Result).
concatenate_lists([List|Rest], Acc, Result) :-
    append(Acc, List, NewAcc),
    concatenate_lists(Rest, NewAcc, Result).

concatenate_n_lists(Lists, Result) :-
    concatenate_lists(Lists, [], Result).

maplist_with_index(_, [], [], _).
maplist_with_index(Pred, [Elem1|Tail1], [Elem2|Tail2], Index) :-
    call(Pred, Elem1, Elem2, Index),
    NextIndex is Index + 1,
    maplist_with_index(Pred, Tail1, Tail2, NextIndex).
...
find_number_and_range(LineNum, [], S, Idx, Start, Store, R) :-
    S \= [] -> (
        End is Idx - 1,
        reverse(S, NList),
        atomic_list_concat(NList, NAtom),
        atom_number(NAtom, Number),
        StartIdx is Start + (LineNum * 140),
        EndIdx is End + (LineNum * 140),
        NextStore = [[Number, StartIdx, EndIdx] | Store], 
        NextStore = R
    ) ; (Store = R).

find_number_and_range(LineNum, [H | T], S, Idx, Start, Store, R) :-
    S \= [] -> 
        (
            is_digit(H) ->
                NextIdx is Idx + 1, find_number_and_range(LineNum, T, [H | S], NextIdx, Start, Store, R)
            ;   (
                    NextIdx is Idx + 1,
                    End is Idx - 1,
                    reverse(S, NList),
                    ...
                    StartIdx is Start + (LineNum * 140),
                    EndIdx is End + (LineNum * 140),
                    NewStore = [[Number, StartIdx, EndIdx] | Store],
                    find_number_and_range(LineNum, T, [], NextIdx, 0, NewStore, R)
                )
        )
    ; 
        (
            is_digit(H) ->
                NextIdx is Idx + 1, find_number_and_range(LineNum, T, [H | S], NextIdx, Idx, Store, R)
            ;   NextIdx is Idx + 1, find_number_and_range(LineNum, T, S, NextIdx, 0, Store, R)
        ).


find_possible_ranges(_, [], Store, _, Store).
find_possible_ranges(LineNum, [H | T], Store, Idx, R) :-
    is_symbol(H) ->
        (
            NextIdx is Idx + 1,
            UpIdxAbs is Idx + ((LineNum - 1) * 140),
            DownIdxAbs is Idx + ((LineNum + 1) * 140),
            LeftIdxAbs is (Idx - 1) + (LineNum * 140),
            RightIdxAbs is (Idx + 1) + (LineNum * 140),
            TopLeftAbs is (Idx - 1) + ((LineNum - 1) * 140),
            TopRightAbs is (Idx + 1) + ((LineNum - 1) * 140),
            BottomLeftAbs is (Idx - 1) + ((LineNum + 1) * 140),
            BottomRightAbs is (Idx + 1) + ((LineNum + 1) * 140),
            (in_range(UpIdxAbs) -> A = [UpIdxAbs] ; A = []),
            (in_range(DownIdxAbs) -> B = [DownIdxAbs] ; B = []),
            (in_range(LeftIdxAbs) -> C = [LeftIdxAbs] ; C = []),
            (in_range(RightIdxAbs) -> D = [RightIdxAbs] ; D = []),
            (in_range(TopLeftAbs) -> E = [TopLeftAbs] ; E = []),
...
parse_nums_and_ranges(S, R, LineNum) :-
    find_number_and_range(LineNum, S, [], 0, 0, [], R).
...
main() :-
    ...
    maplist_with_index(parse_nums_and_ranges, I, Ns, 0),
